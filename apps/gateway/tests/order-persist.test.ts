...
describe("Order & Notification Persistence Flow", () => {
  let server: Server;
  let baseUrl: string;
  let testCustToken: string;

  before(async () => {
    testCustToken = jwt.sign(
      { id: "usr-cust-1", email: "customer@orderly.com", role: "CUSTOMER" },
      JWT_SECRET,
      { expiresIn: "1h" }
    );
  ...
  it("successfully persists an order unit via persistOrderToDb with authoritative total_amount", async () => {
    ...
    const testOrder = {
      ...
      total: 431.9,
      total_amount: 431.9,
      payment_status: "cod_pending" as const,
    ...
    const res = await dbPool.query(
      `SELECT id, subtotal, discount_amount, delivery_fee, platform_fee, tax, total, total_amount, status FROM "Order" WHERE id = $1;`,
      [testOrderId]
    );
  ...
  it("successfully persists a notification WITH an order_id reference", async () => {
    ...
    const res = await dbPool.query(
      `SELECT id, user_id, order_id, title, message, read FROM "Notification" WHERE id = $1;`,
      [notifId]
    );
  ...
  it("POST /orders creates order and persists order + notifications with order_id in PostgreSQL", async () => {
    ...
    const dbOrderRes = await dbPool.query(
      `SELECT id, subtotal, discount_amount, delivery_fee, platform_fee, tax, total, total_amount, status, payment_status FROM "Order" WHERE id = $1;`,
      [createdOrderId]
    );
    ...
    const dbNotifRes = await dbPool.query(
      `SELECT id, user_id, order_id, title, message FROM "Notification" WHERE order_id = $1;`,
      [createdOrderId]
    );
